...
from tracker_ogr.plugin import OgrTrackerPlugin
...
tracker_ogr = Blueprint('tracker_ogr', __name__)
...
class ResourceTrackerOgrView(MethodView):
    ...
    def post(self, id, resource_id):
        ...
        self.ogr.put_on_a_queue(context, 'resource', self.ogr.get_worker().create_resource,
                                _get_pkg_dict(id), _get_res_dict(resource_id), None, None)
        toolkit.redirect_to(u'tracker_ogr.resource', id=id, resource_id=resource_id)
...
class DumpTrackerOgrView(MethodView):
    ...
    def get(self, resource_id):
        ...
        file_format = toolkit.request.GET.get('format', None)
        query_geometry_shape = toolkit.request.GET.get('queryGeometry', None)  # Not easy to verify the WKB value validity.
        query_geometry_srid = toolkit.request.GET.get('srid', None)  # Must correspond a positive integer
        if not file_format:
        ...
        if ogr_response.get("success", False):
            ...
            status, headers, app_iter = None, None, None
            try:
                status, headers, app_iter = toolkit.request.call_application(fileapp)
            except OSError:
                toolkit.abort(404, toolkit._('Exported OGR file not found.'))
            ...
            toolkit.response.headers['Content-Disposition'] = 'attachment; filename={resource_name}.{extension}'.format(
...
tracker_ogr.add_url_rule(u'/dataset/{id}/resource_data/{resource_id}', view_func=ResourceTrackerOgrView.as_view(str(u'resource')))
tracker_ogr.add_url_rule(u'/ogr/dump/{resource_id}', view_func=DumpTrackerOgrView.as_view(str(u'dump')))
